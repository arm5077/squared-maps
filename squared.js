window.Squared = (function(){
	...
	Squared.prototype.make = function(options){
		...
		if( !options ){
			...
			if( options.classes ){
				var localities = options.template.map(function(d){ return d.name });
				// right now this requires a JSON file. Lame, I know
				options.classes.forEach(function(d){
					if( localities.indexOf(d.name) != -1 )
						options.template[ localities.indexOf(d.name) ].class = d.class;
				});
			}
		}
